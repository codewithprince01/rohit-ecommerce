...
const Analytics = () => {
  ...
  const [data, setData] = useState({
    salesData: [],
    categorySales: [],
    summary: { totalRevenue: 0, totalOrders: 0, avgOrderValue: 0, conversionRate: "4.5%" }
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 bg-white dark:bg-gray-800 p-1 rounded-lg border border-gray-100 dark:border-gray-700">
           {["7days", "30days", "1year"].map(p => (
              <button key={p} onClick={() => setPeriod(p)} className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${period === p ? "bg-primary-600 text-white" : "text-gray-500 hover:text-gray-900"}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
         {[
           { label: "Total Revenue", value: `₹${data.summary.totalRevenue.toLocaleString()}`, icon: DollarSign, color: "text-primary-600 bg-primary-50" },
           { label: "Total Orders", value: data.summary.totalOrders, icon: ShoppingBag, color: "text-blue-600 bg-blue-50" },
           { label: "Avg Order Value", value: `₹${data.summary.avgOrderValue.toLocaleString()}`, icon: Target, color: "text-amber-600 bg-amber-50" },
           { label: "Conversion Rate", value: data.summary.conversionRate, icon: Activity, color: "text-purple-600 bg-purple-50" },
         ].map((stat, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
         ...
         <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm flex items-center justify-center">
            <div className="text-center space-y-4">
               <div className="w-16 h-16 bg-primary-50 text-primary-600 rounded-full flex items-center justify-center mx-auto">
                  <TrendingUp size={32} />
               </div>
               <div>
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white">Growing Strong!</h3>
                  <p className="text-sm text-gray-500 max-w-xs mx-auto">Your store's revenue has increased by 12% compared to last week.</p>
               </div>
            </div>
         </div>
      </div>
